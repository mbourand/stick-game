import { v3db, type V3BeatmapEntity } from "./versions/v3";
...
export async function pruneDuplicateBeatmaps(): Promise<void> {
  const all = await v3db.beatmaps.orderBy("createdAt").toArray();
  ...
  if (idsToDelete.length > 0) {
    await v3db.beatmaps.bulkDelete(idsToDelete);
...
async function pruneOrphanedFiles(): Promise<void> {
  const beatmaps = await v3db.beatmaps.toArray();
  ...
  const allFiles = await v3db.files.toArray();
  const orphanIds = allFiles.filter((f) => !referenced.has(f.id)).map((f) => f.id);
  if (orphanIds.length > 0) {
    await v3db.files.bulkDelete(orphanIds);
 ...
 * blobs.
 */
export async function deleteBeatmapAndOrphanedFiles(beatmap: V3BeatmapEntity): Promise<void> {
  await v3db.beatmaps.delete(beatmap.id);
